...
trait HolesGen<T> {
    fn map_ref<U, F: FnMut(&T) -> U>(&self, f: F) -> U;
}
impl<T> HolesDyn<T> for Box<T> {
    fn map_ref<U>(&self, f: &mut dyn FnMut(&T) -> U) -> U {
        f(self)
    }
}
impl<T> HolesGen<T> for Box<T> {
    fn map_ref<U, F: FnMut(&T) -> U>(&self, mut f: F) -> U {
        f(self)
    }
}
...
// ===== algebras =====
trait Alg {
    fn leaf(&self, n: i64) -> i64;
    fn node(&self, a: i64, b: i64) -> i64;
}
struct Count;
impl Alg for Count {
    #[inline]
    fn leaf(&self, _: i64) -> i64 {
        1
    }
    #[inline]
    fn node(&self, a: i64, b: i64) -> i64 {
...
impl Alg for Mix {
    #[inline]
    fn leaf(&self, n: i64) -> i64 {
        n.wrapping_mul(0x9E3779B97F4A7C15u64 as i64) ^ 0x5bd1e995
    }
    #[inline]
    fn node(&self, a: i64, b: i64) -> i64 {
        (a ^ b.rotate_left(13))
...
fn time<F: FnMut() -> i64>(label: &str, nodes: f64, mut f: F) {
    ...
    println!(
        "{label:22} {:8.2} ns/node   ({:.1} ms total)",
        best * 1e9 / nodes,
        best * 1e3
    );
}
...
#[inline(never)]
fn fence_call<A: Alg>(t: &Tree, alg: &A) -> i64 {
    fold_fence(t, alg)
}
